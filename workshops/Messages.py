...
parser.add_argument('-t', '--to', type=str, help="Pomoc komendy. Użycie -d login użytkownika do usunięcia")
parser.add_argument('-s', '--send', type=str, help="Pomoc komendy. Użycie -n login użytkownka do modyfikacji")
...
if args.send and args.user and args.password and args.to and args.send:
    ...
    if loaded_user and check_password(args.password, loaded_user.hashed_password):
        ...
        for m in messages:
            print(m.id, '\t', m.from_name, '\t', m.text, '\t', m.creation_date)
    else:
